...
class BitDanceDiffusionHead(nn.Module):
    ...
    def training_loss(
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            ...
            per_row_rf_loss = (v_target - v_pred).float().square().mean(dim=1)
            loss = per_row_rf_loss.mean()
            per_row_endpoint_mse = (x_pred.float() - x).square().mean(dim=1)

        with torch.no_grad():
            endpoint_mse = per_row_endpoint_mse.mean()
        ...
        if return_per_row:
            stats["_per_row_rf_loss"] = per_row_rf_loss
            stats["_per_row_endpoint_mse"] = per_row_endpoint_mse
        return stats
