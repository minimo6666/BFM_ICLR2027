...
from models.bitdance_binary_diffusion_head import BitDanceDiffusionHead
...
class BinaryDiffusionFlowBitDanceJoint(ExpectationConsistentBFM):
    ...
    def _signed_head_target(self, x_0, x_t):
        """Honor the BFM --p_flip contract for a joint binary target."""
        target = torch.logical_xor(x_0.bool(), x_t.bool()).float() if self.p_flip else x_0
        return target.mul(2.0).sub(1.0)

    def _head_bits_to_clean(self, bits, x_t):
        return torch.logical_xor(bits.bool(), x_t.bool()).float() if self.p_flip else bits
    ...
    def _sample_joint_endpoint(
        ...
        return self._head_bits_to_clean(endpoint_bits.view(b, n, self.codebook_size), x_t)
    ...
    def sample(
        ...
        for i, step_value in enumerate(sampling_steps):
            ...
            if diagnostics is not None:
                channel_prob = x0_joint.float().mean(dim=(0, 1)).clamp(
                    1.0e-7, 1.0 - 1.0e-7
                )
                channel_entropy = -(
                    channel_prob * channel_prob.log()
                    + (1.0 - channel_prob) * (1.0 - channel_prob).log()
                ).mean()
                flat_cells = x0_joint.reshape(-1, x0_joint.shape[-1])
                unique_cell_ratio = (
                    torch.unique(flat_cells, dim=0).shape[0]
                    / float(flat_cells.shape[0])
                )
                diagnostics.append({
                    "outer_t": int(step_value),
                    "xt_one_fraction": float(x_t.mean()),
                    "x0_one_fraction": float(x0_joint.mean()),
                    "x_next_one_fraction": float(x_next.mean()),
                    "x0_joint_channel_entropy_mean": float(channel_entropy),
                    "unique_cell_ratio": float(unique_cell_ratio),
                    "hamming_x_next_vs_x_t": float(
                        (x_next != x_t).float().mean()
                    ),
                    "endpoint_flip_fraction": float((x0_joint != x_t).float().mean()),
                    "endpoint_spatial_variance": float(x0_joint.var(dim=1, unbiased=False).mean()),
                    "next_spatial_variance": float(x_next.var(dim=1, unbiased=False).mean()),
                })
            x_t = x_next
