"""Time-aligned Bernoulli Flow sampler with expectation-consistent X0 marginalization.

This module preserves the physical diffusion time ``t`` for all scheduler and
posterior calculations, but conditions the denoiser on ``t - 1`` during both
training and sampling.  That convention matches the released BLD model while
keeping the same number of physical sampling transitions.
...
class BinaryDiffusionFlowDecouple(nn.Module):
    def __init__(self, H, denoise_fn, mask_id):
        ...
        # Base BFM retention path.  With no environment override this is
        # byte-for-byte the original schedule used by the controlled baseline.
        interpolation_t = torch.linspace(
            1.0, 0.0, steps=self.num_timesteps + 1
        )

        # Optional Barrier-Aware (BA) path.
        #
        # IMPORTANT: this changes the FORWARD probability path itself, and the
        # same interpolation_t is consequently used by q_sample(), the exact
        # cross-step transition, the oracle bridge, training, previews and
        # sampling.  It is NOT posterior tempering and it never substitutes a
        # cavity probability for P(X0|Xt).
        #
        # For physical indices t=1..T:
        #     tau_t = a * (1 - t/T)
        # while tau_0 = 1 exactly.  Therefore
        #     P(X_t^i = X_0^i | X_0) = (1 + tau_t)/2
        #     barrier B_t = 0.5 + 0.5*a*(1 - t/T).
        # a=1 reproduces the original BFM path exactly.  a=0.4 gives
        # B_48=.55, B_40=.575, ..., B_1=.696875 for T=64.
        barrier_scale_raw = os.environ.get("BFM_BARRIER_RETENTION_SCALE")
        if barrier_scale_raw is None:
            self.barrier_retention_scale = 1.0
        else:
            self.barrier_retention_scale = float(barrier_scale_raw)
            if not (0.0 < self.barrier_retention_scale <= 1.0):
                raise ValueError(
                    "BFM_BARRIER_RETENTION_SCALE must be in (0,1], got "
                    f"{self.barrier_retention_scale}"
                )
            if self.barrier_retention_scale != 1.0:
                physical_t = torch.arange(
                    1,
                    self.num_timesteps + 1,
                    dtype=interpolation_t.dtype,
                )
                interpolation_t[1:] = self.barrier_retention_scale * (
                    1.0 - physical_t / float(self.num_timesteps)
                )
                interpolation_t[0] = 1.0

        if not torch.all(interpolation_t[:-1] >= interpolation_t[1:]):
            raise RuntimeError("Barrier-aware retention path is not monotone.")
        if interpolation_t[0].item() != 1.0:
            raise RuntimeError("Clean endpoint tau_0 must equal 1.")
        if abs(float(interpolation_t[-1].item())) > 1e-7:
            raise RuntimeError("Noise endpoint tau_T must equal 0.")
    ...
    def _train_loss(self, x_0, label=None, x_ct=None):
        ...
        if label is not None:
            ...
            raw_logits = self._denoise_fn(
                idx=x_t_in, label=label, time_steps=t - 1
            )
        else:
            raw_logits = self._denoise_fn(x_t_in, time_steps=t - 1)
        ...
        if self.loss_final == "weighted":
            weight = (
                1.0 - ((t - 1).float() / self.num_timesteps)
            ).view(-1, 1, 1)
    ...
    def _pre_reverse_corrector(
        ...
        *,
        ...
        """Optional same-level correction before the reverse predictor.

        The comparison BFM remains unchanged: its hook is the identity.  A
        subclass can override this method to revise ``x_t`` at
        ``physical_time`` before the analytical reverse transition is
        evaluated.  Keeping the hook here avoids requiring a correction kernel
        at the clean hard-final state t=0.
        """
        del physical_time, temp, label, guidance
        return x_t
    ...
    def sample(
        ...
        for i, step_value in enumerate(sampling_steps):
            ...
            x_t = self._pre_reverse_corrector(
                x_t,
                t,
                temp=temp,
                label=label,
            ...
            if (
                ...
                raw_logits = self._denoise_fn(x_t, time_steps=t - 1)
                raw_logits = raw_logits / temp
