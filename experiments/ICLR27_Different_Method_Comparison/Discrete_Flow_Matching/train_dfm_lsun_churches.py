...
import psutil
import gc
def get_dfm_sampler(H):
    """Create the experiment-local Campbell binary DFM sampler."""
    if H.sampler != 'dfm_binary':
        raise ValueError(
            f"This training entry only supports sampler='dfm_binary', got {H.sampler!r}."
        )
    denoise_fn = TransformerBD(H).cuda()
    total_params = sum(parameter.numel() for parameter in denoise_fn.parameters())
    print(f"Binary DFM parameters: {total_params / 1_000_000:.2f} million")
    return BinaryDiscreteFlowModelDecouple(H, denoise_fn, H.codebook_size)


@torch.no_grad()
def save_packed_latent_preview(
        H, sampler, generator, embedding_weight, step, num_images=10):
    ...
    try:
        ...
        print(f"Saved 64-NFE EMA preview at step {step}", flush=True)
    finally:
...
def main(H, vis):
    ...
    sampler = get_dfm_sampler(H).cuda()
    ...
    while True:
        ...
        for batch in train_loader:
            ...
            if not H.distributed or dist.get_rank() == 0:
                if completed_step % H.steps_per_log == 0:
            ...
            if completed_step >= H.train_steps:
                if not H.distributed or dist.get_rank() == 0:
                    print(f"DFM training completed at step {completed_step}.")
                return
