...
def main(H, vis=None):
    ...
    experiment_variant = os.environ.get(
        "EXPERIMENT_VARIANT", "lowrank_sensitivity_v8"
    ).strip().lower()

    if not H.sampler.startswith("flow_"):
        raise ValueError("V8 training expects a flow sampler.")
    if experiment_variant != "lowrank_sensitivity_v8":
        raise ValueError(f"Unknown EXPERIMENT_VARIANT={experiment_variant!r}")

    denoiser = TransformerBDLowRankSensitivityV8(H).to(device)
    sampler_without_ddp = BinaryDiffusionFlowLowRankSensitivityV8(
    ...
    total_params = sum(p.numel() for p in sampler_without_ddp.parameters())
    adapter_params = sampler_without_ddp._denoise_fn.adapter_parameter_count()
    base_params_count = total_params - adapter_params
    log(f"Sampler params total: {total_params / 1e6:.4f}M")
    log(f"Base params: {base_params_count / 1e6:.4f}M")
    log(f"Low-rank adapter params: {adapter_params / 1e6:.6f}M ({100.0 * adapter_params / total_params:.4f}%)")
    log(f"Controlled experiment variant: {experiment_variant}")
    log(f"Experiment seed: {seed}")
    log(
        "V8: original aligned BFM base path + isolated low-rank interval residual; "
        f"rank={sampler_without_ddp._denoise_fn.sensitivity_adapter_rank}; "
        f"lambda={sampler_without_ddp.lr_lambda:g}; "
        f"NFEs={sampler_without_ddp.lr_nfes}; "
        f"NFE_weights={sampler_without_ddp.lr_nfe_weights}"
    )
    log(f"V8 grids: {sampler_without_ddp.lr_grids_by_nfe}")
    log(f"V8 mean raw S^2 by NFE: {sampler_without_ddp.lr_mean_s2_by_nfe}")
    log(
    ...
    while completed_updates < total_updates:
        ...
        for micro_index in range(update_freq):
            ...
            grad_norm = scaler(
                ...
                parameters=sampler_without_ddp.base_parameters_for_clip(),
                create_graph=False,
